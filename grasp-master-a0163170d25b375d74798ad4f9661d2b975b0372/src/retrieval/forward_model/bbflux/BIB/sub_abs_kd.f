...
C
      ...
      END
c----------
      SUBROUTINE DATATM(PATH,IATM,PSURF,UH2O,UO3,                       &
     &                  DONUSER,ALT,P,T,RO,DENS)
C  *********************************************************************
C  * SSPGM DE:LECTURE SUR 'CARTES' OU DISQUE DES PRESSIONS,TEMPERATURES*
C  *                      ET CONCENTRATION                             *
C  *********************************************************************
      PARAMETER(NCORPS=7,NLEVEL=50)
      CHARACTER(*),                    INTENT(IN)   :: PATH
      INTEGER,                         INTENT(IN)   :: IATM
      REAL,                            INTENT(IN)   :: PSURF,UH2O,UO3
      REAL, DIMENSION(NLEVEL,13),      INTENT(OUT)  :: DONUSER
      REAL, DIMENSION(NLEVEL),         INTENT(OUT)  :: ALT, P, T, DENS
      REAL, DIMENSION(NCORPS, NLEVEL), INTENT(OUT)  :: RO
      REAL, DIMENSION(NLEVEL) :: ROH2O,ROO3,RON2O,ROO2,                 &
     &                             ROCH4,ROCO,ROCO2
      REAL,  PARAMETER, DIMENSION(NCORPS) :: ATMOCM=(/3.410E+22,        &
     &                                                1.395E+22,        &
     &                                                1.279E+22,        &
     &                                                1.395E+22,        &
     &                                                2.192E+22,        &
     &                                                3.837E+22,        &
     &                                                1.918E+22/)
      REAL,  PARAMETER, DIMENSION(NCORPS) :: PDSMOL=(/18.0,44.0,48.0,   &
     &                                                44.0,28.0,16.0,   &
     &                                                32.0/)
!......................................................................
!     IF ITAM = 0: User's Profile (for example UserProfile)
!     DONUSER(J,K): for each level J, entry K = 1,13
!     Z(km), P(hPa), T(K), H2O, CO2, O3, N2O, CO, CH4, O2, 
!     density (not used), NO2, SO2 with gas concentrations in ppmv
!......................................................................
      IF (IATM.LE.0) THEN
         OPEN(2,FILE=PATH//'ABS_GAS_DATA/UserProfile')
         DO J=1,NLEVEL
            READ(2,*) I,(DONUSER(J,K),K=1,13)
            ALT(  J)=DONUSER(J, 1)
            IF (PSURF.GT.0.0 .AND. J.EQ.1) DONUSER(1,2)=PSURF
            P(    J)=DONUSER(J, 2)
            T(    J)=DONUSER(J, 3)
            ROH2O(J)=DONUSER(J, 4)
            ROCO2(J)=DONUSER(J, 5)
         ...
         CLOSE(2)
      END IF
!     read profile SO2,NO2
      OPEN(1,FILE=PATH//'ABS_GAS_DATA/SO2-NO2')
      DO J=1,NLEVEL
         READ(1,*) DONUSER(J,13),DONUSER(J,12)
      END DO
      CLOSE(1)
!     read from internal look-up-table
...
!
      IF (IATM.GT.0) THEN
         DO J=1,NLEVEL
            DONUSER(J,1 )=ALT(J)
            IF (PSURF.GT.0. .AND. J.EQ.1) P(1)=PSURF
            DONUSER(J,2 )=P(  J)
            DONUSER(J,3 )=T(  J)
            DONUSER(J,4 )=ROH2O(J)
            DONUSER(J,5 )=ROCO2(J)
      ...
      DO J=1,NLEVEL-1
         DP=P(J)-P(J+1)
         DO K=1,NCORPS
            RO(K,J)=DP*(RO(K,J)+RO(K,J+1))/2.0*ATMOCM(K)
            RO(K,J)=RO(K,J)/6.022E+23*PDSMOL(K)
         END DO
      END DO
!     correction of H2O
      IF (UH2O.GE.0.0) THEN
         QH2O=SUM(RO(1,1:NLEVEL))
         RO(1,1:NLEVEL)     =UH2O/QH2O*RO(1,1:NLEVEL)
         DONUSER(1:NLEVEL,4)=UH2O/QH2O*DONUSER(1:NLEVEL,4)
      END IF
      QH2O=SUM(RO(1,1:NLEVEL))
!     correction of O3
      IF (UO3.GE.0.0) THEN
         QO3=466.23*SUM(RO(3,1:NLEVEL))
         DO J=1,NLEVEL
            RO(3,J)=RO(3,J)*UO3/QO3*0.001
            DONUSER(J,6)=DONUSER(J,6)*UO3/QO3*0.001
         END DO
      END IF
      QO3=466.23*SUM(RO(3,1:NLEVEL))

!!XH   print of water vapor content and ozone content
!      print *, 'H2O (g/cm2)= ',QH2O
!      print *, 'O3 (cm.atm)= ',QO3

!
!      OPEN(3,FILE='BIB/Atm_Profile')
!      WRITE(3,'(a13,1x,f6.3)') 'H2O (g/cm2)= ',QH2O
!      WRITE(3,'(a13,1x,f6.3)') 'O3 (cm.atm)= ',QO3
!      WRITE(3,*) 'LEVEL  H(km)  P(hPa)  T(K)  H2O(ppmv)  CO2(ppmv)  O3 '&
!     &          ,'(ppmv)  N2O(ppmv)  CO (ppmv)  CH4(ppmv)  O2 (ppmv)   '&
!     &          ,' DENSITY  NO2(ppmv)  SO2(ppmv)'
!      DO J=1,NLEVEL
!         WRITE(3,'(1X,I5,1X,F6.1,1X,F7.1,1X,F5.1,11(1X,E10.4))')        &
!     &      J,(DONUSER(J,K),K=1,13)
!      END DO
!      CLOSE(3)
!
      RETURN
      END SUBROUTINE DATATM

c-----------------------------------------------------------------------
      subroutine coefabs(nabs,lamb,ik,prs,tmp,xki,a,b,xk)
c
      parameter(maxkd=15,maxp=23,maxnu=208,ncorps=7,maxt=3)
      dimension p(maxp),tp(maxt)
      dimension xki(ncorps,maxnu,maxp,maxkd)
      dimension a(ncorps,maxnu,maxp,maxkd),b(ncorps,maxnu,maxp,maxkd)

      data tp /190.,260.,330./
      data p /1050.,802.,631.,487.,398.,251.,158.,100.,63.1,39.8,25.1,
     $15.8,10.,6.31,3.98,2.51,1.58,1.,0.631,0.398,0.251,0.067,0.012/

      if(tmp.gt.tp(3))tmp=tp(3)
      if(tmp.lt.tp(1))tmp=tp(1)
      if(prs.le.p(23))then
         xk=0.
         goto 1
      endif
      if(prs.gt.p(1))prs=p(1)
      do l=1,maxp-1
       if(prs.le.p(l).and.prs.ge.p(l+1))lref=l
      enddo

      xk1=xki(nabs,lamb,lref,ik)+a(nabs,lamb,lref,ik)*(tmp-260.)+
     $b(nabs,lamb,lref,ik)*(tmp-260.)**2
      xk2=xki(nabs,lamb,lref+1,ik)+a(nabs,lamb,lref+1,ik)*(tmp-260.)+
     $b(nabs,lamb,lref+1,ik)*(tmp-260.)**2

      xk=xk1+(xk2-xk1)*(prs-p(lref))/(p(lref+1)-p(lref))

 1    return
      end
c-----------------------------------------------------------------------
C
      subroutine read_absN(path,nabs,jabs,nexp,al,xki,a,b)
C !! NEW CODE !! new subroutine
c
      parameter(maxkd=15,maxp=23,maxnu=208,ncorps=7)
      dimension nexp(ncorps,maxnu),al(ncorps,maxnu,maxkd)
      dimension xki(ncorps,maxnu,maxp,maxkd)
      dimension a(ncorps,maxnu,maxp,maxkd),b(ncorps,maxnu,maxp,maxkd)
      character(*) :: path
C
       do il=1,maxnu
        ...
        al(nabs,il,1)=1.0
        do ip=1,maxp
         xki(nabs,il,ip,1)=0.0
        enddo
       enddo
       if(jabs.eq.0)goto 2
C
C  Lecture des coefficients
C !! NEW CODE !! new files
      if(nabs.eq.1)open(1,file=path//'ABS_GAS_DATA/coefN_H2O')
      if(nabs.eq.2)open(1,file=path//'ABS_GAS_DATA/coefN_CO2')
      if(nabs.eq.3)open(1,file=path//'ABS_GAS_DATA/coefN_O3')
 1    read(1,*,end=2)lamb,nexpo
C !! NEW CODE !! new spectral resolution (index definition)
      if(lamb.lt.14400.)il=int((lamb-2500.)/100.)+1
      if(lamb.ge.14400.)il=int((lamb-14400.)/400.)+120
      if(nexpo.ne.0)then
       nexp(nabs,il)=nexpo
       read(1,*)(al(nabs,il,ik),ik=1,nexp(nabs,il))
       do ip=1,maxp
        read(1,*)ii,(xki(nabs,il,ip,ik),ik=1,nexp(nabs,il))
       enddo
       do l=1,maxp
        read(1,*)(a(nabs,il,l,k),k=1,nexp(nabs,il))
        read(1,*)(b(nabs,il,l,k),k=1,nexp(nabs,il))
       enddo
      endif
      goto 1
...
c
      ...
      SUBROUTINE CONTINUUMNEW(WV,NLEVEL,ICONT,DONUSER,TAUCONT)

      DOUBLE PRECISION WV

      DIMENSION DONUSER(NLEVEL,13),ICONT(7),TAUCONT(NLEVEL-1)

c     DATA ATMOCM/3.410E+22,1.395E+22,1.279E+22,1.395E+22,2.192E+22,
c    13.837E+22,1.918E+22/
...
C EN hPa, km et K
      DPRES=DONUSER(L,2)-DONUSER(L+1,2)
      ALT1=DONUSER(L,1)
      ALT2=DONUSER(L+1,1)
      PR=(DONUSER(L,2)+DONUSER(L+1,2))/2.0
      TP=(DONUSER(L,3)+DONUSER(L+1,3))/2.0

C EN PPMV
      ROH2O=(DONUSER(L,4)+DONUSER(L+1,4))/2.0*1.0E-06
      RON2=0.7809
      ROO2=(DONUSER(L,10)+DONUSER(L+1,10))/2.0*1.0E-06

C EN MOL/CM2
      ROCO2=(DONUSER(L,5)+DONUSER(L+1,5))/2.0*1.0E-06*44/28.97
     $      *DPRES*1.395E+22
      ROO3=(DONUSER(L,6)+DONUSER(L+1,6))/2.0*1.0E-06*48/28.97
     $      *DPRES*1.279E+22
      RONO2=(DONUSER(L,12)+DONUSER(L+1,12))/2.0*1.0E-06*46/28.97
     $       *DPRES*1.3340E+22
      ROSO2=(DONUSER(L,13)+DONUSER(L+1,13))/2.0*1.0E-06*64/28.97
     $       *DPRES*9.5884E+21

      IF(ICONT(1).EQ.1)CALL CONT_H2O(WV,ROH2O,DPRES,PR,TP,ALT1,ALT2,
     $                 TAUH2O)
      IF(ICONT(2).EQ.1)CALL CONT_CO2(WV,ROCO2,PR,TP,TAUCO2)
      IF(ICONT(3).EQ.1)CALL CONT_O3(WV,ROO3,TP,TAUO3)
      IF(ICONT(4).EQ.1)CALL CONT_N2(WV,PR,TP,RON2,ALT1,ALT2,ROH2O,TAUN2)
      IF(ICONT(5).EQ.1)CALL CONT_O2(WV,PR,TP,ROO2,ALT1,ALT2,ROH2O,TAUO2)
      IF(ICONT(6).EQ.1)CALL CONT_NO2(WV,RONO2,TAUNO2)
      IF(ICONT(7).EQ.1)CALL CONT_SO2(WV,ROSO2,TAUSO2)

      TAUCONT(L)=TAUH2O+TAUCO2+TAUO3+TAUO2+TAUN2+TAUNO2+TAUSO2
...
C
C
      SUBROUTINE CONT_H2O(WV,ROH2O,DPRES,PR,TP,ALT1,ALT2,TAUH2O)
...
C       SCH2O=0.
C       IF(CS0.GT.0)THEN
C         SCH2O=CS0*(CS/CS0)**((TP-TREF)/(260.-TREF))
C         ALPHA2=200**2
C         ALPHS2= 120.**2
C         FACTRS= 0.15
C         BETAS = 5.E-06
...
C         VS2 = (WV-V0S)**2
C         SFAC = 1.
C         IF (WV.GE.700. .AND.  WV.LE.1200.) THEN
C              JFAC = (WV-700.)/10. + 0.00001
C              SFAC = XFAC(JFAC)
C         ENDIF
C         SFAC = SFAC*(1.+0.3   *(1.E+04/((WV)**2+1.E+04))) *
C    $         (1.-0.2333*(ALPHA2/((WV-1050.)**2+ALPHA2))) *
C    $         (1.-FACTRS*(ALPHS2/(VS2+(BETAS*VS2**2)+ALPHS2)))
C         SCH2O = SFAC * SCH2O
C       ENDIF
C       write(6,*)'NU=',WV
C       write(6,*)'SELF OLD=',SCH2O
C
C--------------------------------------------------------------------
C                             SELF NEW
C
...
C        VF2 = (WV-V0F)**2
C        VF6 = VF2 * VF2 * VF2
C        FSCAL  = (1.-FACTRF*(HWSQF/(VF2+(BETAF*VF6)+HWSQF)))
C        VF2 = (WV-V0F2)**2
C        VF4 = VF2*VF2
C        FSCAL = FSCAL* (1.- 0.6*(HWSQF2/(VF2 + BETA2*VF4 + HWSQF2)))
C        FCH2O=CF*FSCAL
C       write(6,*)'FOREIGN OLD=',FCH2O
C
C--------------------------------------------------------------------
C                             FOREIGN NEW
C
...
C
 ...
 1    RETURN
      ...
      SUBROUTINE CONT_CO2(WV,ROCO2,PR,TP,TAUCO2)

      IMPLICIT REAL*8           (V)
      DOUBLE PRECISION WV

      COMMON /FCO2/ V1S,V2S,DVS,NPTS,S(1003)
      DATA PREF,TREF /1013.25,296.0/

      TAUCO2=0.
      IF(WV.GE.10000.)RETURN
      ICT=INT(WV/10.)+3
      CCO2=S(ICT)
      XCNTCO2=CCO2*1.0E-20*PR/PREF*TREF/TP*WV*1.0005
      TAUCO2=XCNTCO2*ROCO2

      RETURN
      END


      SUBROUTINE CONT_O3(WV,ROO3,TP,TAUO3)

      DOUBLE PRECISION WV

      COMMON /O3HUV/  V1S,V2S,DVS,NPTS,S(133)
      COMMON /O3CHAP/ V1O1,V2O1,DVO1,NPT1,O3CH(55)
      COMMON /O3HH0/ V10S,V20S,DV0S,NPTS0,S0(2687)
      COMMON /O3HH1/ V11S,V21S,DV1S,NPTS1,S1(2690)
      COMMON /O3HH2/ V12S,V22S,DV2S,NPTS2,S2(2690)

C--------------------------------------------------------------------
C                     PARAMETRES D'ENTREE
C--------------------------------------------------------------------
C  TP    ---------> TEMPERATURE DE LA COUCHE
C--------------------------------------------------------------------
      TAUO3=0.0
C--------------------------------------------------------------------
...
C--------- CONTINUUM DE LA BANDE D'ABSORPTION DE CHAPPUIS -----------
      ...
      CNTO3CH=0.
      IF(WV.LT.9170.0.OR.WV.GT.24565.0)GOTO 1
      V=WV
      CALL O3CHAPW(V,C0,CT1,CT2)
      TC=TP-273.15
      CNTO3CH=(C0+CT1*TC+CT2*TC*TC)*1.0E-20
 1    CONTINUE

C--------- CONTINUUM DE HARTLEY HUGGINS DANS L'UV -------------------

      CNTO3UV=0.
      IF(WV.LT.40800.0)GOTO 2
      IUV=INT((WV-40800.0)/100)+1
      CNTO3UV=(S(IUV+1)-S(IUV))/100.0*(WV-(40800.0+100.0*
     $             (IUV-1)))+S(IUV)
...
C--------- CONTINUUM DE HARTLEY HUGGINS -----------------------------
      ...
      CNTO3HH=0.
      IF(WV.LT.27370.0.OR.WV.GE.40800.0)GOTO 3
      IHH=INT((WV-27370.0)/5)+1
      TC=TP-273.15
      CNTO3HH=S0(IHH)*(1.+TC*S1(IHH)+TC*TC*S2(IHH))*1.0E-20
 3    CONTINUE

      TAUO3=(CNTO3UV+CNTO3CH+CNTO3HH)*ROO3

      RETURN
      END


      SUBROUTINE CONT_NO2(V,RONO2,CROSS)                                
...
C        THIS CHECK PREVENTS N FROM EXCEEDING NMAX IN THE ALGORITHM BELO
      ...
      ENDIF                                                             
      CROSS = CROSS * RONO2 / 2.6868E+19
      RETURN                                                            
      END                                                               



      SUBROUTINE CONT_SO2(V,ROSO2,CROSS)                                
...
C        THIS CHECK PREVENTS N FROM EXCEEDING NMAX IN THE ALGORITHM BELO
      ...
      ENDIF                                                             
      CROSS = CROSS * ROSO2 / 2.6868E+19
      RETURN                                                            
      END                                                               



      SUBROUTINE CONT_N2(V,PR,TP,RON2,ALT1,ALT2,ROH2O,C4L)
...
C
      ...
      COMMON /C4C8/ C4(157)                                             
      DATA PREF,TREF,XLOSMT /1013.25,296.0, 2.6868E+19/
C **  N2 CONTINUUM                                                      
      C4L=0.                                                            
      IF(V.LT.2020.) RETURN                                             
      IF(V.GT.2800.) RETURN                                             
      ...
      L=(IV-2020)/5+1                                                   
      WTOT=XLOSMT*273.15/TP*PR/PREF*(ALT2-ALT1)*1.0E+05
      RON2=RON2*WTOT/(1+ROH2O)
      C4L=C4(L)*(PR/PREF*TREF/TP)*RON2*1.0E-05*V                        
      RETURN                                                            
      END                                                               
...
C                                                                       
      COMMON /C4C8/  C4(157)                                            
C                                                                       
C     DATA V1N2,V2N2,DVN2,NPTN2 / 2020., 2800., 5.0, 157 /              
C                                                                       
      DATA C4    /                                                      
     + 0.       , 2.859E-30, 5.718E-30, 1.144E-29, 2.288E-29, 4.573E-29,
     + 9.011E-29, 1.803E-28, 3.605E-28, 7.210E-28, 1.442E-27, 2.884E-27,
     A 5.683E-27, 7.491E-27, 9.876E-27, 1.273E-26, 1.717E-26, 2.057E-26,
     B 2.506E-26, 3.309E-26, 4.342E-26, 5.223E-26, 6.427E-26, 7.555E-26,
     C 1.030E-25, 1.356E-25, 1.697E-25, 2.131E-25, 2.565E-25, 3.131E-25,
     D 3.645E-25, 4.017E-25, 4.389E-25, 4.890E-25, 5.392E-25, 5.857E-25,
     E 6.237E-25, 6.714E-25, 7.191E-25, 7.814E-25, 8.437E-25, 9.082E-25,
     F 9.592E-25, 1.022E-24, 1.086E-24, 1.140E-24, 1.194E-24, 1.247E-24,
     G 1.282E-24, 1.320E-24, 1.357E-24, 1.400E-24, 1.443E-24, 1.498E-24,
     H 1.533E-24, 1.609E-24, 1.685E-24, 1.903E-24, 2.115E-24, 2.397E-24,
     I 2.644E-24, 2.783E-24, 2.940E-24, 2.783E-24, 2.627E-24, 2.383E-24,
     J 2.112E-24, 2.043E-24, 1.992E-24, 1.957E-24, 1.923E-24, 1.923E-24,
     K 1.882E-24, 1.882E-24, 1.899E-24, 1.933E-24, 1.916E-24, 1.899E-24,
     L 1.825E-24, 1.792E-24, 1.708E-24, 1.658E-24, 1.591E-24, 1.507E-24,
     M 1.431E-24, 1.356E-24, 1.265E-24, 1.166E-24, 1.075E-24, 1.009E-24,
     N 8.988E-25, 8.089E-25, 7.352E-25, 6.534E-25, 6.126E-25, 5.717E-25,
     O 5.004E-25, 4.276E-25, 4.036E-25, 3.551E-25, 3.148E-25, 2.825E-25,
     P 2.552E-25, 2.233E-25, 1.914E-25, 1.675E-25, 1.515E-25, 1.436E-25,
     Q 1.261E-25, 1.104E-25, 1.025E-25, 9.457E-26, 8.672E-26, 7.488E-26,
     R 6.232E-26, 5.842E-26, 5.452E-26, 4.676E-26, 3.896E-26, 3.506E-26,
     S 3.081E-26, 2.850E-26, 2.618E-26, 2.465E-26, 2.310E-26, 2.310E-26,
     T 2.345E-26, 2.284E-26, 2.239E-26, 2.041E-26, 1.903E-26, 1.614E-26,
     U 1.364E-26, 1.134E-26, 9.653E-27, 7.664E-27, 6.082E-27, 5.060E-27,
     V 4.258E-27, 3.455E-27, 2.889E-27, 2.338E-27, 1.951E-27, 1.519E-27,
     W 1.189E-27,                                                       
     + 5.877E-28, 2.940E-28, 1.470E-28, 7.289E-29, 3.675E-29, 1.837E-29,
     + 9.086E-30, 4.543E-30, 2.271E-30, 1.136E-30, 5.679E-31, 0.       /
C                                                                       
      END                                                               
C
C
      SUBROUTINE CONT_O2(V,PR,TP,ROO2,ALT1,ALT2,ROH2O,TAUO2)

      DOUBLE PRECISION V

      COMMON/O2C/ O2DRAY(74),O2C001(74),O2S0(74),O2A(74),O2B(74),       
     *            V1O2,V2O2,DVO2,NPTO2                                  
      DATA XLOSMT / 2.68675E+19 /, RO2 / 0.20946 /
      DATA PREF,TREF /1013.25,296.0/

      TAUO2=0.
      IF(V.LT.1395.OR.V.GT.1760)GOTO 1
      WTOT=XLOSMT*273.15/TP*PR/PREF*(ALT2-ALT1)*1.0E+05
      ROO2=ROO2*WTOT/(1+ROH2O)
      TD = TP-220.00
      I=(V-1395.)/5+1
      TAUO2 = O2S0(I)*EXP(O2A(I)*TD+O2B(I)*TD*TD)/(RO2*V)
      WO2 = (PR/PREF)*ROO2/XLOSMT
      TAUO2 = TAUO2*WO2*0.78*V
...
C
     ...
     $                  DENS)
      ...
      PARAMETER(NLEVEL=50)
      DIMENSION P(NLEVEL),T(NLEVEL),ALT(NLEVEL)
      DIMENSION ROH2O(NLEVEL),ROO3(NLEVEL),RON2O(NLEVEL),ROO2(NLEVEL)
      DIMENSION ROCH4(NLEVEL),ROCO(NLEVEL),ROCO2(NLEVEL)
      DIMENSION P1(NLEVEL),T1(NLEVEL),ALT1(NLEVEL)
      DIMENSION ROH2O1(NLEVEL),ROO31(NLEVEL),RON2O1(NLEVEL)
      DIMENSION ROCH41(NLEVEL),ROCO1(NLEVEL),ROCO21(NLEVEL)
      DIMENSION ROO21(NLEVEL),DENS(NLEVEL),DENS1(NLEVEL)
...
C--------- CONTINUUM DE LA BANDE D'ABSORPTION DE CHAPPUIS -----------
      ...
      CNTO3CH=0.
      IF(FREQ.LT.9170.0.OR.FREQ.GT.24565.0)GOTO 1
      V=FREQ
      CALL O3CHAPW(V,C0,CT1,CT2)
      TC=TEMP-273.15
      CNTO3CH=(C0+CT1*TC+CT2*TC*TC)*1.0E-20
...
C--------- CONTINUUM DE HARTLEY HUGGINS DANS L'UV -------------------
      ...
      CNTO3UV=0.
      IF(FREQ.LT.40800.0)GOTO 2
      IUV=INT((FREQ-40800.0)/100)+1
      CNTO3UV=(S(IUV+1)-S(IUV))/100.0*(FREQ-(40800.0+100.0*
     $             (IUV-1)))+S(IUV)
...
C--------- CONTINUUM DE HARTLEY HUGGINS -----------------------------
      ...
      CNTO3HH=0.
      IF(FREQ.LT.27370.0.OR.FREQ.GE.40800.0)GOTO 3
      IHH=INT((FREQ-27370.0)/5)+1
      TC=TEMP-273.15
      CNTO3HH=S0(IHH)*(1.+TC*S1(IHH)+TC*TC*S2(IHH))*1.0E-20
 3    CONTINUE

      ABSCNTO3=CNTO3UV+CNTO3CH+CNTO3HH

      RETURN
      END

      SUBROUTINE CONTHLBL(FREQ,CS0,CS,CF)

      IMPLICIT REAL*8 (V)
      DOUBLE PRECISION FREQ

      COMMON /SH2O/ V1S1,V2S1,DVS1,NPTS1,SH2OT0(2003)
      COMMON /S260/ V1S2,V2S2,DVS2,NPTS2,SH2OT1(2003)
      COMMON /FH2O/ V1S3,V2S3,DVS3,NPTS3,FH2O(2003)
C********    WATER    ********                                          
C                                                                       
C                                                                       
      ICONT=INT(FREQ/10.)+3
      ...
      BCF=FH2O(ICONT+1)
C
      CS0=(BCS0-ACS0)*(FREQ-(ICONT-3)*10)/10.+ACS0
      CS=(BCS-ACS)*(FREQ-(ICONT-3)*10)/10.+ACS
      CF=(BCF-ACF)*(FREQ-(ICONT-3)*10)/10.+ACF
c     CS0=ACS0
c     CS=ACS
c     CF=ACF
C
      ...
      END
C
      SUBROUTINE CNTCO2LBL(FREQ,XCNTCO2,PR,TP)
      ...
      ICT=INT(FREQ/10.)+3
      CCO2=S(ICT)
      XCNTCO2=CCO2*1.0E-20*PR*TP*FREQ*1.0005
...
C
      Y1V=EXP(ALOG(1.0036*3.159E-8)-(2.75E-4)*V)
      Y2V=EXP(ALOG(8.97E-6)-(1.300E-3)*V)
C
      YA=Y1V
      YB=Y2V
      YAINV=1/YA
